...
export class Game extends Scene {
  ...
  setDirectionTexture(sprite, dx, dy, baseName) {
    if (Math.abs(dx) > Math.abs(dy)) {
      sprite.setTexture(`${baseName}-${dx > 0 ? 'east' : 'west'}`);
    } else {
      sprite.setTexture(`${baseName}-${dy > 0 ? 'south' : 'north'}`);
    }
  }
  ...
  create() {
    ...
    this.followingCampers = new Set();

    this.physics.add.overlap(this.girl, this.camper1, () => {
      this.followingCampers.add(this.camper1);
    });
    this.physics.add.overlap(this.girl, this.camper2, () => {
      this.followingCampers.add(this.camper2);
    });
  ...
  update(time, delta) {
    ...
    for (const camper of this.followingCampers) {
      this.physics.moveToObject(camper, this.girl, 100);
    }
  }
